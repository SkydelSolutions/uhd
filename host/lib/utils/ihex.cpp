...
void ihex_reader::to_bin_file(const std::string &bin_filename)
{
    ...
    this->read(boost::bind(&_file_writer_callback, output_file, boost::placeholders::_3, boost::placeholders::_4));
...
std::vector<uint8_t> ihex_reader::to_vector(const size_t size_estimate)
{
    ...
    this->read(boost::bind(&_vector_writer_callback, boost::ref(buf), boost::placeholders::_3, boost::placeholders::_4));
