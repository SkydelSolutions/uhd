            ...
            try{
                ...
                if(_oc_dict[oc].gps and _oc_dict[oc].gps->gps_detected()){
                    ...
                    _tree->create<std::string>(oc_path / "gps_sync_source_mode")
                      .set_publisher(boost::bind(&gps_ctrl::get_sync_source_mode, _oc_dict[oc].gps))
                      .add_coerced_subscriber(boost::bind(&gps_ctrl::set_sync_source_mode, _oc_dict[oc].gps, _1));
                }
                else{
